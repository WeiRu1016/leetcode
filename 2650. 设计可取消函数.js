...
var cancellable = function(generator) {
    ...
    const promise = new Promise((resolve, reject) => {
        const each = (value) => {
            let result;
            if (throwError) {
                result = generator.throw(throwError);
                throwError = null;
            } else {
            ...
            return Promise.resolve(result.value).catch((err) => { throwError = err; }).then(each).catch(reject);
        }
        return each();
    });
...
[
    // { fun: function*() { return 42; }, cancelAt: 100 },
    // { fun: function*() { const msg = yield new Promise(res => res("Hello")); throw `Error: ${msg}`; } },
    // { fun: function*() { yield new Promise(res => setTimeout(res, 200)); return "Success"; }, cancelAt: 100 },
    // { fun: function*() { let result = 0; yield new Promise(res => setTimeout(res, 100)); result += yield new Promise(res => res(1)); yield new Promise(res => setTimeout(res, 100)); result += yield new Promise(res => res(1)); return result; } },
    // { fun: function*() { let result = 0; try { yield new Promise(res => setTimeout(res, 100)); result += yield new Promise(res => res(1)); yield new Promise(res => setTimeout(res, 100)); result += yield new Promise(res => res(1)); } catch(e) { return result; } return result; }, cancelAt: 150 },
    // {
    //     fun: function*() {
    //         try {
    //             yield new Promise((resolve, reject) => reject("Promise Rejected"));
    ...
    //     }
    // }
].forEach(({ fun, cancelAt }) => {
    const [cancel, promise] = cancellable(fun());
    if (cancelAt != null) {
        setTimeout(cancel, cancelAt);
    }
    promise.then((v) => {
        console.log('done:', JSON.stringify({ resolve: v }));
    }).catch((e) => {
        console.log('done:', JSON.stringify({ rejected: e }))
    });
})
// function* tasks() {
//     let result = 0;
